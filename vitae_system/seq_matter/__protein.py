...
class Protein(ESeq[E_Protein]):
    ...
    def calc_metabolic(self) -> float:
        ...
        total = len(self.__seq)
        add_count = sum(1 for aa in self.__seq if aa in ADD_AMINO_ACIDS)
        inhibit_count = sum(1 for aa in self.__seq if aa in INHIBIT_AMINO_ACIDS)

        raw_rate = (add_count - inhibit_count) / total
        rate = min(max(raw_rate, MIN_METABOLIC), MAX_METABOLIC)
        return rate
